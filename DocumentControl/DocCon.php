...
$FileCount++;
  if ($First3 == $CPOPre) {
    $CPONum = preg_replace('/cpo/', '', $FileName); 
    $CPO = 'CCPO'.$CPONum;
//if ($ENABLE_MYSQL == '1') {
...
/// / ------------------------------
if ($MSSQL_SUPPORT == '1') {
$ServerName = $ServerName.'/'.$UserAccount;
$ConnectionInfo = array( "DBName"=>"dbName", "$DBUser"=>"username", "$DBPass"=>"password");
$Connect = sqlsrv_connect( $ServerName, $ConnectionInfo );
if($Connect === false) {
  die( print_r( sqlsrv_errors(), true)); }

$sql = "SELECT cust_po_no  FROM sales_no";
$stmt = sqlsrv_query( $Conn, $sql );
if($stmt === false) {
  echo nl2br ('!!! ERROR !!! DC155, $Time, Could not connect to MSSQL database.
    Check your database status and settings.'."\n");
  die(print_r(sqlsrv_errors(), true) ); }
while($row = sqlsrv_fetch_array( $stmt, SQLSRV_FETCH_ASSOC) ) {
    $CPONumRaw = $row['cust_po_no'];
    $SO = 'SO'.$row['sales_no']; }
    sqlsrv_free_stmt($stmt); } 
  $NEWFileName = ('CPO'.$CPONumRaw.$FileType);
  ...
  if (!file_exists($NEWPathName1)) {
    ...
    $txt = ('OP-Act: '."Copied$acttxt$FileName to $NEWPathName on $Time".'. DC172.');
    $LogFile = file_put_contents($SesLogDir.'/'.$Date.'_'.$LogInc.'.txt', $txt.PHP_EOL , FILE_APPEND); 
...
if ($InputLoc !== $ActionLoc) {
$txt = 'Control OP (Pending) on '.$Time;
$CacheFile = file_put_contents($InstLoc.'/AppCache/OPCache.txt', $txt.PHP_EOL , FILE_APPEND); }
if ($InputLoc == $ActionLoc) {
$txt = 'Control OP (Action) on '.$Time;
$CacheFile = file_put_contents($InstLoc.'/AppCache/OPCache.txt', $txt.PHP_EOL , FILE_APPEND); }
...
if (($BALANCEFileCount) > 0 ) {
    $txt = ('Control Operation Complete! Processed '.$FileCount.' files on '.$Time.'. Left '.$BALANCEFileCount.' action items that still require attention. DC159.');
    $LogFile = file_put_contents($SesLogDir.'/'.$Date.'_'.$LogInc.'.txt', $txt.PHP_EOL , FILE_APPEND); 
