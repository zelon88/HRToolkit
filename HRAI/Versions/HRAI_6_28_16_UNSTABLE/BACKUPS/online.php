...
function getServIdent(){
    ...
    require('/var/www/html/HRProprietary/HRAI/CallForHelp.php');
    return $serverID; }
...
if (file_exists('/var/www/html/HRProprietary/HRAI/core.php')) {
    $node0URL = ("http://localhost/HRProprietary/HRAI");
    ...
    $nodeCachetxt = '/var/www/html/HRProprietary/HRAI/Cache/nodeCache.txt';
    require ($nodeCache);
    ...
    $nodeCache0 = fopen("$nodeCachetxt", "a+");    
    $txt = ('$node0ServerID = '.$node0ServerID.'; $node0OnlineStatus = '.$node0OnlineStatus.'; $node0Busy = '.$node0Busy.'; $node0URL = \''.$node0URL.'\'; ');
    $compLogfile = file_put_contents($nodeCachetxt, $txt.PHP_EOL , FILE_APPEND); } 
...
function getNode($nodeURL){
  ...
  // / If the target URL contains a nodeCache, we open it's contents and save them in our own temp location. 
    if (remoteFileExists($nodeURLCachefile) == true){
    $tmpnodeCachedat = file_get_contents($nodeURLCachefile);
    $tmpnodeCachedat2 = file_put_contents($mytmpnodeCache, $tmpnodeCachedat);
    // / We need to get the variables from our nodeCache, so we're going to include it now.
    include_once $mytmpnodeCache;  
