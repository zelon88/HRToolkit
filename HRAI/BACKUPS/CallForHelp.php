...
require_once '/var/www/html/HRProprietary/HRAI/coreFunc.php';
...
if (isset($_POST['serverIDCFH'])) {
 if ($CallForHelp !== 0) {
  // / If we cannot find other servers to help us we keep going anyway.
    $sesLogfileO = fopen("$sesLogfile", "a+");
    echo nl2br("  $CallForHelp nodes found on local network. \r");
    echo nl2br("  Continuing with expanded resources. \r");
    $txt = ('CallForHelp: Tried 9 common addresses on local network on '.$date.'. Found '.$CallForHelp.' HRAI servers. Continuing with expanded resources... ');
    $compLogfile = file_put_contents($sesLogfile, $txt.PHP_EOL , FILE_APPEND); } }